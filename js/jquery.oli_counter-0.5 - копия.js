(function( $ ){
	...
	$.fn.oli_counter = function(options){
...
// number of signs in each date group
			...
			obj.find('.counter__group').each(function(){
				...
				for(var k=0; k < si[i]; k++){
					group.prepend('<div><span>0</span></div>');
				}	
				i++;
			});
			
// убираем лишние цифры, возникающие из-за незавершенной анимации
			// obj.find('.counter__group div').each(function(){
				// while($(this).find('span').length > 1){
					// $(this).find('span:last-child)').remove();
				// }
			// });
			
			oc_changes = setInterval(function(){
		...
		return daddy.each(function(){
			initCounter($(this));
		});
		
	}
	$.fn.oli_counterDestroy = function(){
        $.fn.oli_counter=function(){
			oc_changes = clearInterval;
		};
    };
